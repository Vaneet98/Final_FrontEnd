  ...
  const getCountries = async () => {
    try {
      const response = await axios.get(
        `http://localhost:4002/department/getDepartmentSalarylevel3`
      );
      console.log("This level1 &&&&&&&&&&&&&&&&&&&&&&&&7v",response.data.data);
      setCountries(response.data.data.user2.rows);
      setFiltercountries(response.data.data.user2.rows);
    } catch (error) {
      console.log(error);
    }
  };
