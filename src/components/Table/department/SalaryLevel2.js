  ...
  const getCountries = async () => {
    try {
      const response = await axios.get(
        `http://localhost:4002/department/getDepartmentSalarylevel2`
      );
      console.log(response);
      setCountries(response.data.data.user1.rows);
      setFiltercountries(response.data.data.user1.rows);
    } catch (error) {
      console.log(error);
    }
  };
