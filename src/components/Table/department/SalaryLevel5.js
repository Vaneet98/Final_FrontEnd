  ...
  const getCountries = async () => {
    try {
      const response = await axios.get(
        `http://localhost:4002/department/getDepartmentSalarylevel5`
      );
      console.log(response);
      setCountries(response.data.data.user4.rows);
      setFiltercountries(response.data.data.user4.rows);
    } catch (error) {
      console.log(error);
    }
  };
